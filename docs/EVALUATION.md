...
The framing that explains why this matters: **Codex is the patch worker; Studio is the delivery runtime around it.** Coding agents (Codex, Claude Code, Cursor) can write code unattended for hours — what they don't do is decide which patches are safe to land, record why each decision was made, or produce a schema-validated audit trail a reviewer can second-guess later. **The model is not the system. The delivery loop is the system.** The 12-rule Promotion Gate, the 10-rule Apply Gate, the per-run package, the change-mode artifact contract, the human-in-the-loop review queue, the schema validators, the provenance commit trailers — those are the system. Codex is one component of it.
