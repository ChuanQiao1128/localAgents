#!/usr/bin/env bash
# RC-3F: Third-party Detector Adapter Probe.
#
# Goal: Add the AI Writing Naturalizer's first real third-party AI detector
# adapter (Sapling), with mock-default + real-opt-in routing, on top of
# the RC-3E LLMOps eval suite. Adapter is observe-only this milestone:
# real Sapling calls are NEVER inside integration; only the operator's
# post-run `npm run detect:real` step exercises the real network path
# (and only if SAPLING_API_KEY is set in the env file).
#
# NO real OpenAI / Anthropic / any LLM API call. NO LLM-as-judge.
# NO numpy / pandas / sklearn / langchain / ragas / deepeval / evals.
# NO new ML pip deps. NO automatic rewrite loop. NO `while score >
# threshold` logic. NO detector score as deployment gate.
#
# IMPORTANT: detector real-mode is observe-only.
# - eval / detector are NOT in `npm run build` chain (build remains
#   `npm run backend:test && prisma generate && next build`)
# - eval / detector are NOT in agent-studio integration commands
# - detector failure does NOT block deploy in this milestone
# - this script runs `npm run detect:real` ONCE post-autonomous-run as
#   verification + reports the artifact path; failure is surfaced but
#   does not change the exit status of the autonomous run itself
#
# DEFAULT IS DRY-RUN. Pass --run to actually execute.
# --run will:
#   - npm install (~30-60s)
#   - python venv + pip install (cached after first run)
#   - burn ~75-150k Codex tokens (3 tasks × 1 candidate cap)
#   - create a real Vercel preview deployment for the Next.js frontend
#   - run `npm run detect:real` once after the autonomous run completes
#     (mock-only by default; real Sapling only if SAPLING_API_KEY env set)
#
# Required env (loaded from ~/.local-agent-vercel.env):
#   VERCEL_TOKEN / VERCEL_ORG_ID / VERCEL_PROJECT_ID
#
# Optional env:
#   SAPLING_API_KEY                  if set AND DETECTOR_PROVIDER=sapling,
#                                    enables real-mode for the post-run
#                                    `npm run detect:real` step.
#                                    Integration ALWAYS uses mock.
#
# Required binaries on PATH:
#   $CODEX_BIN (default /opt/homebrew/bin/codex), vercel, npm, python3
#
# IMPORTANT: VERCEL_PROJECT_ID should point at a NEW Vercel project
# `rc3f-detector-adapter-probe`. Use the corrected env-rewrite recipe
# from docs/rc3f-prep-report.md (do NOT `source` old env file before
# `cat >`-ing the new one — that's the standing RC-3B/C gotcha).
#
# Hard NO-list (same as rc2c.sh / rc3a.sh / rc3b.sh / rc3c.sh / rc3d.sh /
# rc3e.sh):
#   - NOT production deploy
#   - NOT rollback
#   - NOT --dangerously-bypass-approvals-and-sandbox / --yolo
#   - tokens never echoed; only token_present=true persists in artifacts

set -euo pipefail

# -----------------------------------------------------------------------------
# Config
# -----------------------------------------------------------------------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
DOGFOOD_REPO="$REPO_ROOT/.dogfood/rc3f-detector-adapter-probe"
WORKSPACE="${WORKSPACE:-/tmp/rc3f-real}"
CODEX_BIN="${CODEX_BIN:-/opt/homebrew/bin/codex}"
VERCEL_ENV_FILE="${VERCEL_ENV_FILE:-$HOME/.local-agent-vercel.env}"

BUDGET_MAX_TASKS=3
BUDGET_MAX_INNER_RUNS=3
BUDGET_MAX_CANDIDATES=1
BUDGET_MAX_REPAIR=1
BUDGET_MAX_ABANDONED=1
BUDGET_MAX_CORRECTIVE=1

INTEGRATION_TIMEOUT_SEC=1200

# -----------------------------------------------------------------------------
# Argv
# -----------------------------------------------------------------------------
RUN_MODE="dry-run"
for arg in "$@"; do
  case "$arg" in
    --run) RUN_MODE="run" ;;
    --dry-run) RUN_MODE="dry-run" ;;
    -h|--help)
      cat <<'HELP'
rc3f.sh — RC-3F Third-party Detector Adapter Probe

Usage:
  scripts/rc3f.sh                       # dry-run: print commands only
  scripts/rc3f.sh --run                 # execute (real Codex + real Vercel)
  CODEX_BIN=/path/to/codex scripts/rc3f.sh --run

Env:
  CODEX_BIN          path to codex binary (default: /opt/homebrew/bin/codex)
  WORKSPACE          agent-studio workspace (default: /tmp/rc3f-real)
  VERCEL_ENV_FILE    Vercel env file (default: ~/.local-agent-vercel.env)
  SAPLING_API_KEY    optional; enables real Sapling mode in post-run
                     `npm run detect:real`. Integration always uses mock.

Defaults to dry-run. --run will:
  - run `npm install` then `python3 -m venv backend/.venv` + pip install
  - consume ~75-150k Codex tokens (3 tasks × 1 candidate cap)
  - create a real Vercel preview deployment for the Next.js frontend
  - run `npm run detect:real` once post-autonomous-run as verification
HELP
      exit 0
      ;;
    *) echo "rc3f.sh: unknown arg '$arg' (try --help)" >&2; exit 2 ;;
  esac
done

# -----------------------------------------------------------------------------
# Helpers
# -----------------------------------------------------------------------------
say()   { printf '\n[rc3f] %s\n' "$*"; }
warn()  { printf '\n[rc3f WARN] %s\n' "$*" >&2; }
do_or_print() {
  if [[ "$RUN_MODE" == "run" ]]; then
    printf '\n$ %s\n' "$*"
    eval "$*"
  else
    printf '\n[dry-run] $ %s\n' "$*"
  fi
}

# -----------------------------------------------------------------------------
# Pre-flight
# -----------------------------------------------------------------------------
say "RC-3F Third-party Detector Adapter Probe — mode: $RUN_MODE"
say "repo:        $REPO_ROOT"
say "dogfood:     $DOGFOOD_REPO"
say "workspace:   $WORKSPACE"
say "codex bin:   $CODEX_BIN"
say "vercel env:  $VERCEL_ENV_FILE"

if [[ ! -x "$CODEX_BIN" ]]; then
  warn "CODEX_BIN '$CODEX_BIN' is not executable. Try: which codex"
  exit 1
fi
say "codex --version: $("$CODEX_BIN" --version 2>&1 | head -1)"

if ! command -v vercel >/dev/null 2>&1; then
  warn "vercel CLI not on PATH. Install via: npm i -g vercel"
  exit 1
fi
say "vercel --version: $(vercel --version 2>&1 | head -1)"

if ! command -v npm >/dev/null 2>&1; then
  warn "npm not on PATH (required for npm install setup step)"
  exit 1
fi
say "npm --version: $(npm --version 2>&1 | head -1)"
say "node --version: $(node --version 2>&1 | head -1)"

if ! command -v python3 >/dev/null 2>&1; then
  warn "python3 not on PATH (required for backend venv + pytest + detector CLI)"
  exit 1
fi
say "python3 --version: $(python3 --version 2>&1 | head -1)"

if [[ ! -f "$VERCEL_ENV_FILE" ]]; then
  warn "VERCEL_ENV_FILE not found at $VERCEL_ENV_FILE"
  exit 1
fi
# shellcheck disable=SC1090
source "$VERCEL_ENV_FILE"

missing=()
for var in VERCEL_TOKEN VERCEL_ORG_ID VERCEL_PROJECT_ID; do
  if [[ -z "${!var:-}" ]]; then
    missing+=("$var")
  fi
done
if [[ ${#missing[@]} -gt 0 ]]; then
  warn "Vercel env missing: ${missing[*]}"
  exit 1
fi
say "Vercel env: VERCEL_TOKEN/ORG_ID/PROJECT_ID all present (values not echoed)"
if [[ -n "${SAPLING_API_KEY:-}" ]]; then
  say "Sapling key: present (value not echoed). Post-run detect:real may run real-mode."
else
  say "Sapling key: not set. Post-run detect:real will use mock fallback (mode=mock, error.code=missing_api_key if DETECTOR_PROVIDER=sapling)."
fi

if [[ ! -d "$DOGFOOD_REPO" ]]; then
  warn "dogfood repo missing at $DOGFOOD_REPO"
  exit 1
fi

# -----------------------------------------------------------------------------
# Cost / blast radius warning
# -----------------------------------------------------------------------------
cat <<'WARN'

------------------------------------------------------------------------------
RC-3F COST + BLAST-RADIUS WARNING

  - npm install:    ~30-60s; ~300-350 MB node_modules.
  - pip install:    ~30-60s first time; cached thereafter (no new deps —
                    Codex MUST NOT add openai / anthropic / langchain /
                    ragas / deepeval / evals / numpy / pandas / sklearn /
                    requests / aiohttp / urllib3. httpx is already
                    available for the Sapling adapter.)
  - Codex tokens:   ~75-150k (3 tasks × 1 candidate cap).
  - Wall clock:     ~5-10 min Codex + ~60-120s setup + ~30-90s Vercel
                    deploy + ~few seconds for `npm run detect:real`
                    post-run (mock) or ~1-3s post-run (real Sapling
                    if key set).
  - Vercel:         ONE preview deployment for the Next.js frontend.
                    The FastAPI backend is NOT deployed in RC-3F.
                    Use a NEW Vercel project: rc3f-detector-adapter-probe.
                    MUST have Deployment Protection / Vercel Authentication
                    DISABLED, same as rc3a/b/c/d/e.
                    Seed vercel.json is frontend-only; if `vercel link`
                    re-injects experimentalServices, that's a known
                    observation — record but do NOT activate backend deploy.
  - Backend deploy: NOT touched.
  - Production:     NOT touched.
  - Rollback:       NOT enabled.
  - Real LLM:       NOT called. cost_usd field in detector artifact is 0.0.
  - LLM-as-judge:   NOT used.
  - Real Sapling:   ONLY in post-run `npm run detect:real`, AND ONLY if
                    SAPLING_API_KEY is set in the env file. Integration
                    always uses mock provider. Detector failure is
                    observe-only; never blocks build/deploy.
  - Detector loop:  NEVER. RC-3F is the adapter-shape probe. The
                    detector-informed rewrite flow (capped 2-3 rounds)
                    belongs to RC-3G.

Dry-run prints commands only. 0 tokens, 0 deploys, 0 detector calls.
------------------------------------------------------------------------------
WARN

# -----------------------------------------------------------------------------
# Plan
# -----------------------------------------------------------------------------
say "Plan:"
echo "  1. Wipe workspace at $WORKSPACE"
echo "  2. agent-studio init"
echo "  3. agent-studio new --from $DOGFOOD_REPO/requirements.md"
echo "  4. Seed project with RC-3E-completed baseline + RC-3F additions:"
echo "       package.json, configs, vercel.json (frontend-only override),"
echo "       .gitignore, .env, .env.example, app/, prisma/, backend/ (incl."
echo "       data/detector/runs/.gitkeep + scripts/detect.sh + scripts/eval.sh)"
echo "  5. Write RC-3F agent-studio.yaml (deploy.enabled=true preview, no rollback)"
echo "  6. npm install in the project"
echo "  7. git init + baseline commit"
echo "  8. autonomous preflight"
echo "  9. autonomous start (real Codex × 3 → real Vercel preview frontend → real smoke)"
echo " 10. autonomous status / logs --tail 80 / reviews list"
echo " 11. autonomous validate-artifacts --json"
echo " 12. POST-RUN: npm run detect:real in the workspace project dir + print artifact"
echo " 13. Print: 3 commit hashes + deployment URL + smoke status + detector artifact path"

# -----------------------------------------------------------------------------
# Step 1-4: workspace + project setup
# -----------------------------------------------------------------------------
do_or_print "rm -rf '$WORKSPACE'"
do_or_print "mkdir -p '$WORKSPACE'"
do_or_print "'$REPO_ROOT/agent-studio' --root '$WORKSPACE' init"
do_or_print "'$REPO_ROOT/agent-studio' --root '$WORKSPACE' new --from '$DOGFOOD_REPO/requirements.md'"

if [[ "$RUN_MODE" == "run" ]]; then
  PROJECT_DIR="$(ls -d "$WORKSPACE/.agent-studio/projects/"*/ 2>/dev/null | head -1 | sed 's:/$::')"
  if [[ -z "${PROJECT_DIR:-}" ]]; then
    warn "no project dir created"
    exit 1
  fi
  say "project dir: $PROJECT_DIR"
else
  PROJECT_DIR="<workspace>/.agent-studio/projects/ai-writing-humanizer-XXXXXX"
  say "project dir (dry-run placeholder): $PROJECT_DIR"
fi

# Copy seed files. Use rsync-style explicit list so a stray file in
# DOGFOOD_REPO can't sneak in.
for item in package.json next.config.mjs tsconfig.json tailwind.config.ts postcss.config.mjs .gitignore vercel.json .env .env.example; do
  do_or_print "cp '$DOGFOOD_REPO/$item' '$PROJECT_DIR/'"
done
# Optional lockfile — copy if present so `npm ci` can run deterministically.
if [[ -f "$DOGFOOD_REPO/package-lock.json" ]]; then
  do_or_print "cp '$DOGFOOD_REPO/package-lock.json' '$PROJECT_DIR/'"
fi
do_or_print "cp -r '$DOGFOOD_REPO/app' '$PROJECT_DIR/'"
do_or_print "cp -r '$DOGFOOD_REPO/prisma' '$PROJECT_DIR/'"
do_or_print "cp -r '$DOGFOOD_REPO/backend' '$PROJECT_DIR/'"
do_or_print "cp -r '$DOGFOOD_REPO/scripts' '$PROJECT_DIR/'"

# -----------------------------------------------------------------------------
# Step 5: RC-3F agent-studio.yaml
# Same shape as RC-3E. The integration command list is derived by the
# runtime from package.json scripts (typecheck/build/test). Detector
# CLI is opt-in via npm run detect:real, NOT in build chain.
# -----------------------------------------------------------------------------
YAML_BODY="$(cat <<YAML
agentic:
  patch_worker: codex
  codex:
    command: $CODEX_BIN
    sandbox: workspace-write
    ask_for_approval: on-request
    timeout_sec: $INTEGRATION_TIMEOUT_SEC
    max_prompt_chars: 80000

autonomous:
  budgets:
    max_tasks_per_session: $BUDGET_MAX_TASKS
    max_total_inner_runs: $BUDGET_MAX_INNER_RUNS
    max_candidates_per_task: $BUDGET_MAX_CANDIDATES
    max_repair_attempts_per_candidate: $BUDGET_MAX_REPAIR
    max_abandoned_tasks: $BUDGET_MAX_ABANDONED
    max_corrective_tasks: $BUDGET_MAX_CORRECTIVE

integration:
  every_n_tasks: 1
  run_at_session_end: true
  timeout_sec: $INTEGRATION_TIMEOUT_SEC

deploy:
  enabled: true
  target: vercel
  environment: preview
  project_path: "."
  vercel:
    prod: false
    prebuilt: false
    build_before_deploy: false
    inspect: true
    inspect_timeout: "5m"
    token_env: "VERCEL_TOKEN"
    org_id_env: "VERCEL_ORG_ID"
    project_id_env: "VERCEL_PROJECT_ID"
  smoke_checks:
    enabled: true
    timeout_sec: 10
    retries: 0
    checks:
      - name: home
        method: GET
        path: /
        expected_status: 200
  rollback:
    enabled: false
    production_only: true
    trigger_on_smoke_failure: false
YAML
)"

if [[ "$RUN_MODE" == "run" ]]; then
  printf '%s\n' "$YAML_BODY" > "$PROJECT_DIR/agent-studio.yaml"
  say "wrote $PROJECT_DIR/agent-studio.yaml"
else
  say "agent-studio.yaml that will be written:"
  printf '%s\n' "$YAML_BODY" | sed 's/^/    /'
fi

# Belt-and-suspenders safety
if [[ "$RUN_MODE" == "run" ]]; then
  if grep -qE '^  environment: production' "$PROJECT_DIR/agent-studio.yaml"; then
    warn "environment=production detected — refusing"
    exit 1
  fi
  if grep -qE '^    prod: true' "$PROJECT_DIR/agent-studio.yaml"; then
    warn "vercel.prod=true detected — refusing"
    exit 1
  fi
fi

# -----------------------------------------------------------------------------
# Step 6: install Node deps. Python venv is created lazily by
# scripts/backend-test.sh on first integration.
# -----------------------------------------------------------------------------
if [[ -f "$DOGFOOD_REPO/package-lock.json" ]]; then
  do_or_print "(cd '$PROJECT_DIR' && npm ci --no-audit --no-fund)"
else
  do_or_print "(cd '$PROJECT_DIR' && npm install --no-audit --no-fund)"
fi

# -----------------------------------------------------------------------------
# Step 7: git baseline
# -----------------------------------------------------------------------------
do_or_print "(cd '$PROJECT_DIR' && git init -q -b main)"
do_or_print "(cd '$PROJECT_DIR' && git config user.email rc3f@dogfood)"
do_or_print "(cd '$PROJECT_DIR' && git config user.name rc3f)"
do_or_print "(cd '$PROJECT_DIR' && git add -A)"
do_or_print "(cd '$PROJECT_DIR' && git -c commit.gpgsign=false commit -q -m 'rc3f baseline')"

# -----------------------------------------------------------------------------
# Step 8: preflight
# -----------------------------------------------------------------------------
do_or_print "'$REPO_ROOT/agent-studio' --root '$WORKSPACE' autonomous preflight"

if [[ "$RUN_MODE" != "run" ]]; then
  say "DRY-RUN finished. Re-run with --run to actually consume tokens + deploy."
  exit 0
fi

# -----------------------------------------------------------------------------
# Step 9-11: real run (only --run)
# -----------------------------------------------------------------------------
say "STARTING autonomous run — Codex tokens + real Vercel deploy ahead."
say "Press Ctrl+C in the next 5s to abort..."
sleep 5

do_or_print "'$REPO_ROOT/agent-studio' --root '$WORKSPACE' autonomous start"
do_or_print "'$REPO_ROOT/agent-studio' --root '$WORKSPACE' autonomous status"
do_or_print "'$REPO_ROOT/agent-studio' --root '$WORKSPACE' autonomous logs --tail 80"
do_or_print "'$REPO_ROOT/agent-studio' --root '$WORKSPACE' autonomous reviews list"
do_or_print "'$REPO_ROOT/agent-studio' --root '$WORKSPACE' autonomous validate-artifacts --json"

# -----------------------------------------------------------------------------
# Step 12: POST-RUN detect:real verification (NEW vs RC-3E).
# Runs `npm run detect:real` inside the workspace project dir. Failure is
# surfaced (set +e + capture exit code) but does NOT abort the script —
# detector is observe-only this milestone.
# -----------------------------------------------------------------------------
say "POST-RUN: running npm run detect:real as observe-only verification..."
DETECT_EXIT=0
if [[ -d "$PROJECT_DIR" ]]; then
  ( cd "$PROJECT_DIR" && npm run detect:real ) || DETECT_EXIT=$?
  if [[ "$DETECT_EXIT" -eq 0 ]]; then
    say "npm run detect:real: exit 0 (observe-only pass)"
  else
    warn "npm run detect:real: exit $DETECT_EXIT (observe-only — does NOT block this script)"
  fi

  # Print latest detector artifact path + key fields
  DETECT_ARTIFACT="$(ls -t "$PROJECT_DIR/backend/data/detector/runs/"*/detector-result.json 2>/dev/null | head -1 || true)"
  if [[ -n "${DETECT_ARTIFACT:-}" ]]; then
    say "detector artifact: $DETECT_ARTIFACT"
    python3 - <<PY || true
import json
with open("$DETECT_ARTIFACT") as fh:
    d = json.load(fh)
print(f"  schema_version:     {d.get('schema_version')}")
print(f"  detector_run_id:    {d.get('detector_run_id')}")
print(f"  provider / mode:    {d.get('provider')} / {d.get('mode')}")
print(f"  score / label:      {d.get('score')} / {d.get('label')}")
print(f"  confidence:         {d.get('confidence')}")
print(f"  latency_ms:         {d.get('latency_ms')}")
print(f"  cost_usd:           {d.get('cost_usd')}")
err = d.get("error")
if err:
    print(f"  error:              code={err.get('code')} message={err.get('message')}")
else:
    print(f"  error:              null")
ss = d.get("sentence_scores") or []
print(f"  sentence_scores:    {len(ss)} entries")
PY
  else
    warn "no detector artifact found under $PROJECT_DIR/backend/data/detector/runs/"
  fi
else
  warn "PROJECT_DIR missing; skipping post-run detect:real"
fi

# -----------------------------------------------------------------------------
# Step 13: post-run summary
# -----------------------------------------------------------------------------
say "Per-task git history:"
(cd "$PROJECT_DIR" && git log --oneline -10) || true

SESS_DIR="$(ls -d "$PROJECT_DIR/.agent/autonomous/sessions/"*/ 2>/dev/null | head -1 | sed 's:/$::')"
if [[ -n "${SESS_DIR:-}" ]]; then
  DEPLOY_JSON="$(ls -t "$SESS_DIR/deployments/"*/deployment.json 2>/dev/null | head -1 || true)"
  SMOKE_JSON="$(ls -t "$SESS_DIR/smoke-checks/"*/smoke-check.json 2>/dev/null | head -1 || true)"
  if [[ -n "${DEPLOY_JSON:-}" ]]; then
    say "deployment.json: $DEPLOY_JSON"
    python3 -c "import json; d=json.load(open('$DEPLOY_JSON')); print(f\"  status: {d.get('status')}\"); print(f\"  url:    {d.get('deployment_url')}\")" || true
  fi
  if [[ -n "${SMOKE_JSON:-}" ]]; then
    say "smoke-check.json: $SMOKE_JSON"
    python3 -c "import json; s=json.load(open('$SMOKE_JSON')); print(f\"  status: {s.get('status')}\"); print(f\"  failure: {s.get('failure')}\")" || true
  fi
fi

cat <<'POST'

------------------------------------------------------------------------------
POST-RUN TRIAGE — RC-3F

  A. session completed + 3 commits + deployment.status=ready +
     smoke.status=passed + validate-artifacts ok=true + 0 reviews +
     `npm run detect:real` exit 0 + valid detector artifact written
     → RC-3F verified. Detector adapter shape works end-to-end.
     → Next milestone: RC-3G (Detector-Informed Rewrite Flow — separate
       scoping; capped 2-3 rounds with composite scoring).

  B. backend:test fails on a task → real failure surface. Allowed fixes
     per spec: prompt / context-pack / repair-loop / eval-wiring /
     changed-files classification. Likely culprits:
       - Codex imported requests / aiohttp / urllib3 instead of httpx
         → tighten task-002 prompt; do NOT add the dep
       - Sapling URL/key hardcoded → tighten task-002 wording
       - Raw response leaked API key (redaction failure) → tighten
         task-001 redaction test + task-002 raw-store path
       - Mock detector nondeterministic (random/time) → tighten
         task-001 acceptance
       - Missing SAPLING_API_KEY raised 500 instead of mock-fallback
         → check task-002 endpoint code; fallback is the spec
       - Codex modified package.json / detect.sh / eval.sh / pytest.ini
         (out-of-scope) → tighten requirements.md scope corrections
       - Codex added detect:real to build chain → tighten "NOT in build"
       - Frontend tries to fetch /detect at runtime → tighten task-003
       - Codex added GPTZero / Originality adapters too → tighten
         "one provider per milestone" wording
       - Codex added rewrite loop based on detector score → COMPLIANCE
         BOUNDARY VIOLATION; reject and tighten requirements.md

  C. Codex / Vercel env / SSO failure → no product code change; fix env
     (likely needs a fresh rc3f Vercel project + auth disabled, same
     operator pattern as rc3a/b/c/d/e — see prep doc for CORRECTED
     env-rewrite recipe).

  D. Detector CLI exits non-zero (POST-RUN) → observe-only this
     milestone. The script surfaces the warning and the artifact path;
     does NOT fail. Inspect the artifact JSON to see error.code. If
     SAPLING_API_KEY is set, network errors against real Sapling are
     possible; those are operator/network issues, not RC-3F bugs.
------------------------------------------------------------------------------
POST
