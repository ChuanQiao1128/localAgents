#!/usr/bin/env bash
# RC-3C: Next.js + Tailwind + Prisma + FastAPI mock LLM processing pipeline probe.
#
# Goal: prove Local Agent Dev Studio can drive a SECOND-language /
# SECOND-service shape end-to-end. Codex × 3 tasks:
#   - task-001 + task-002 modify the FastAPI backend (Python)
#   - task-003 modifies the Next.js frontend
# All gated by the same `npm run build` (which runs
# `npm run backend:test && prisma generate && next build`) — failure
# in pytest fails the build, which fails integration AND Vercel deploy.
#
# NO real LLM calls / NO RAG / NO real network beyond pip install +
# Vercel deploy / NO backend deployment / NO auth / NO Stripe.
#
# DEFAULT IS DRY-RUN. Pass --run to actually execute.
# --run will:
#   - npm install (~30-60s; pulls Next.js + Prisma + frontend deps)
#   - python venv + pip install (~30-60s first time; pip-cached
#     thereafter; Prisma 5.22.0 + pydantic + pytest + httpx)
#   - burn ~75-150k Codex tokens (3 tasks × 1 candidate cap; Python
#     patches may run slightly larger than schema patches)
#   - create a real Vercel preview deployment (frontend only;
#     backend is local-only validation)
#
...
# (e.g. `rc3c-fastapi-processing-probe`). The corrected env-rewrite
# recipe (per RC-3B operator gotcha) is documented in
# docs/rc3c-prep-report.md operator pre-checklist.
#
# Hard NO-list (same as rc2c.sh / rc3a.sh / rc3b.sh):
...
DOGFOOD_REPO="$REPO_ROOT/.dogfood/rc3c-fastapi-processing-probe"
WORKSPACE="${WORKSPACE:-/tmp/rc3c-real}"
CODEX_BIN="${CODEX_BIN:-/opt/homebrew/bin/codex}"
VERCEL_ENV_FILE="${VERCEL_ENV_FILE:-$HOME/.local-agent-vercel.env}"

# Same conservative budgets as RC-3A/B's clean passes.
BUDGET_MAX_TASKS=3
...
# Bumped from RC-3A/B's 900s to 1200s. Build chain is now
# `backend:test && prisma generate && next build` — pytest + venv
# bootstrap on first run could push integration past 900s.
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      cat <<'HELP'
rc3c.sh — RC-3C FastAPI + mock LLM processing pipeline probe

Usage:
  scripts/rc3c.sh                       # dry-run: print commands only
  scripts/rc3c.sh --run                 # execute (real Codex + real Vercel)
  CODEX_BIN=/path/to/codex scripts/rc3c.sh --run

Env:
  CODEX_BIN          path to codex binary (default: /opt/homebrew/bin/codex)
  WORKSPACE          agent-studio workspace (default: /tmp/rc3c-real)
...
HELP
      exit 0
      ;;
    *) echo "rc3c.sh: unknown arg '$arg' (try --help)" >&2; exit 2 ;;
  esac
done
...
# Pre-flight
# -----------------------------------------------------------------------------
say "RC-3C FastAPI + mock LLM processing pipeline probe — mode: $RUN_MODE"
say "repo:        $REPO_ROOT"
...
------------------------------------------------------------------------------
RC-3C COST + BLAST-RADIUS WARNING

  - npm install:    ~30-60s; ~300-350 MB node_modules (Next + Prisma + ...).
  - pip install:    ~30-60s first time (~80 MB venv: fastapi + pydantic +
                    pytest + httpx). Pip-cached thereafter.
  - Codex tokens:   ~75-150k (3 tasks × 1 candidate cap).
  - Wall clock:     ~5-10 min Codex + ~60-120s setup + ~30-90s Vercel
                    deploy (now also runs backend:test inside `npm run build`,
                    adds ~30-60s to deploy).
  - Vercel:         ONE preview deployment for the Next.js frontend.
                    The FastAPI backend is NOT deployed in RC-3C.
                    The `rc3c-fastapi-processing-probe` Vercel project
                    MUST have Deployment Protection / Vercel Authentication
                    DISABLED, same as rc3a/b — or smoke will 401.
  - Backend deploy: NOT touched.
  - Production:     NOT touched.
  - Rollback:       NOT enabled.
  - Real LLM:       NOT called. Codex's task scope is "deterministic mock
                    processor"; if Codex tries to import openai / anthropic /
                    litellm / etc, integration will fail (no such package
                    in requirements.txt) and that's expected behavior.
...
echo "  4. Seed project with Next.js + Prisma + FastAPI starter:"
echo "       package.json, configs, vercel.json, .gitignore, .env, app/,"
echo "       prisma/, backend/, scripts/"
echo "  5. Write RC-3C agent-studio.yaml (deploy.enabled=true preview, no rollback)"
echo "  6. npm install in the project (Next + Prisma deps; backend venv built"
echo "     lazily by the first npm run backend:test)"
...
# -----------------------------------------------------------------------------
# Step 5: RC-3C agent-studio.yaml
# Same shape as RC-3B. The integration command list is derived by the
# runtime from package.json scripts (typecheck/build/test); we don't
# have a YAML-side `required_commands` knob. Backend tests are gated
# by being bundled into the `build` script (per package.json).
# -----------------------------------------------------------------------------
YAML_BODY="$(cat <<YAML
agentic:
  patch_worker: codex
...
# -----------------------------------------------------------------------------
# Step 6: install Node deps. The Python venv is created lazily by
# scripts/backend-test.sh when the first integration runs `npm run build`.
# -----------------------------------------------------------------------------
if [[ -f "$DOGFOOD_REPO/package-lock.json" ]]; then
...
do_or_print "(cd '$PROJECT_DIR' && git config user.email rc3c@dogfood)"
do_or_print "(cd '$PROJECT_DIR' && git config user.name rc3c)"
do_or_print "(cd '$PROJECT_DIR' && git add -A)"
do_or_print "(cd '$PROJECT_DIR' && git -c commit.gpgsign=false commit -q -m 'rc3c baseline')"
...
------------------------------------------------------------------------------
POST-RUN TRIAGE — RC-3C

  A. session completed + 3 commits + deployment.status=ready +
     smoke.status=passed + validate-artifacts ok=true + 0 reviews
     → RC-3C verified. Next.js + Prisma + FastAPI two-language probe works.
     → Next milestone: RC-3D (Style Guide RAG).
  ...
  B. backend:test fails on a task → real failure surface. Allowed fixes per spec:
     ...
     classification. Likely culprits to look at first:
       - Codex imported openai / anthropic / litellm despite mock-only
         instruction → integration shows ImportError; tighten prompt
       - Pydantic v1 vs v2 syntax mismatch → check pydantic==2.10.4 declared
       - Codex put backend code outside backend/ → out_of_scope
       - venv setup script flaky → check scripts/backend-test.sh stderr
       - `*.pyc` / __pycache__ leaked into changed-files → analogous to
         RC-3A.7 tsbuildinfo gap; runtime may need another suffix filter
       - frontend task tried to fetch the backend at runtime even though
         it's documentation-only → out-of-scope import
       - Vercel build failed on backend:test step → check Vercel build env
         has python3 (it does on Vercel Build Image v2) and that pip can
         install fastapi/pytest/etc

  C. Codex / Vercel env / SSO failure → no product code change; fix env
     (likely needs a fresh rc3c-fastapi-processing-probe Vercel project +
     auth disabled, same operator pattern as rc3a/b — see prep doc for
     CORRECTED env-rewrite recipe).
------------------------------------------------------------------------------
POST
